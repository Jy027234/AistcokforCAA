...
def default_registry() -> SourceRegistry:
    return SourceRegistry([
        ...
        SourceSpec(
            ...
            integration_state=IntegrationState.PLANNED,
            health=SourceHealth.BLOCKED,
            rights=dict(_UNKNOWN_RIGHTS),
            notes=[
                "2026-09-19 使用者确认付费权限不可接受，当前产品不采用",
                "历史探针：Token 有效，但三项财报接口均返回 40203 无权限",
                "保留登记仅为决策追溯，不进入当前候选链",
            ],
        ),
        SourceSpec(
            source_id="mootdx-tdx",
            display_name="通达信财务包（mootdx）",
            domains=frozenset({Domain.DAILY_QUOTES, Domain.ADJUSTMENTS,
                               Domain.FINANCIALS}),
            priority=36,
            cost_model="FREE_NONCOMMERCIAL",
            requires_credentials=False,
            # 文档含财报公告日期，但历史修订链与公告时刻尚未证明。
            pit_available="PARTIAL",
            pit_basis="RECONSTRUCTED",
            integration_state=IntegrationState.PLANNED,
            ...
            rights=dict(_UNKNOWN_RIGHTS),
            notes=[
                "当前免费覆盖与交叉核对 Spike；不作为已通过的生产源",
                "文档字段覆盖归母净利润、经营现金流、归母权益、收入和财报公告日期",
                "12 只 × 8 期累计值、权益和公告日期覆盖良好",
                "单季字段存在 0 与缺失混淆；300750 年报包出现无版本标识重复行",
                "每个报告期只有一个包，不能重放修订链；条款仍待验收",
            ],
        ),
        SourceSpec(
            source_id="sina-financial",
            display_name="新浪财经三表网页",
            domains=frozenset({Domain.FINANCIALS}),
            priority=37,
            cost_model="FREE_UNDOCUMENTED_HTML",
            requires_credentials=False,
            # 页面只有报告期，没有可依赖的公告日期或历史修订链。
            pit_available="NO",
            pit_basis="RECONSTRUCTED",
            integration_state=IntegrationState.PLANNED,
            health=SourceHealth.HEALTHY,
            rights=dict(_UNKNOWN_RIGHTS),
            notes=[
                "免费结构化数值候选；当前只用于受限 Spike 和交叉核对",
                "8 家普通工商企业 10/10 期完整；银行、保险、证券需独立行业模板",
                "缺失值可保留为缺失，不像通达信单季字段那样与 0 混同",
                "没有可靠公告日期或修订链，必须与巨潮/交易所证据显式配对",
            ],
        ),
    ])
