...
> 生成时间：2026-09-19T12:39:27.693740+00:00
...
| A02 | passed | offline_contract | manifest doctor 通过（未证明 live reload） |
...
| A09 | passed | live_topology | 可信用户确认令牌经 agentctl 到达产品冻结复核，账户变化后的陈旧确认被拒且计划状态未写入；普通模型凭证不可见 |
...
真实拓扑覆盖：10/13 通过；未覆盖：A01, A06, A14；已执行失败：无。
