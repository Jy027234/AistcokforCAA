...
`run_at_local` 的“本地”固定为 `Asia/Shanghai`（UTC+08:00），API 与 worker 共用
同一个显式市场时钟。它不读取容器或宿主的系统时区，避免容器默认 UTC 时把 20:30
推迟到北京时间次日 04:30。
...
这里固定的是逐只行情窗口。采集器同时复用上证指数请求保存一份独立的长交易日历，
发布时写入快照，用于精确执行 120 个交易日上市年龄门槛。扩充日历不会触发 900 只
证券的历史行情重采；严格发布要求日历至少覆盖 120 个交易日。
...
试运行预检还会核对决策快照的 `published_at`：它必须早于执行日 09:30（北京时间）。
