...
### T6 基线与模拟账本（M2）—— 🟡 **核心链已用真实双快照跑通，阶段验收未完**
...
- **当前证据**：S01–S10 与新增时间语义、令牌篡改/重放、卖出批次落库、错误现金阻断发布用例通过；
  默认 `pytest` 无额外路径配置即可运行。2026-09-23 使用
  `snap-eod-2026-09-22-ee07b71a6aa24604` 决策、
  `snap-eod-2026-09-23-6cf37479fa3b48ec` 执行的真实闭环为 **36/36 PASS_WITH_SKIPS**：
  覆盖模型与人工组合预览、一次性确认、模拟成交、日终估值和逐项对账；分红子项因缺少
  2026-06-26 同源 EOD 快照按规则跳过。详细留痕见 `deploy/agentctl-q0/real-flow-result.json`。
- **待关闭**：61 日真实样本贯通、多日模拟与复盘、进程重启后的账本重建；
  把分红权益接入持久化应收/到账链。
...
- **当前状态**：实时 API 已接通研究卡、自选、预览、确认、冻结、执行、估值、对账与决策日志；
  ...
  模型原方案、人工最终方案及差异。冻结／已执行计划可从数据库恢复，估值严格绑定请求的 EOD
  快照并持久化快照、执行计划与 `as_of`。双快照两日、跨进程重启的合成验收 16/16 通过；
  2026-09-18 决策快照到 2026-09-21 执行快照的真实 CLI 流程通过 36/36 项，分红项因缺
  同源 EOD 快照跳过。工作台手工浏览器验收仍待独立验证。
- **对应**：主文档 §19 M3、§5
...
| T7 M3 工作台 | 🟡 | 六页导航；写链路、刷新恢复与双快照合成多日验收通过；真实双快照 CLI 流程见 `deploy/agentctl-q0/real-flow-result.json`，手工浏览器验收未完成 |
| T8 Q1 适配层 + 只读能力 | 🟡 | 研究卡、事件证据、实验提交、模拟预览均已 live 实跑；A05/A07/A08 通过，A06 因模型请求未完成而未覆盖 |
| T9 Q3 预览与确认 | 🟡 | 计划生命周期、人工计划、差异留痕与双快照时点绑定已接入；决策快照在执行日 09:30 前发布，2026-09-21 真实流程已核对 S1 候选、`MODIFY_MODEL`、冻结和模拟成交；分红项因缺同源 EOD 快照跳过，手工浏览器验收未完成 |
...
3. ~~**生产双快照尚缺一个后续日终发布物**~~（已关闭）——真实目录实测中，
   当前 `snap-eod-2026-09-18-longcal-20260919` 已在 9 月 21 日开盘前发布，
   含 900 只证券、65 日行情、535 日交易日历与 900 行旧 `f10-v1` 因子（其中 832 行曾有值）；该因子版本现已因 TTM 公式错误撤回，数量仅作历史事实，不再代表有效研究覆盖，
   ...
   2026-09-23 已进一步用 9 月 22 日决策快照与 9 月 23 日执行快照完成 **36/36**
   项真实闭环验收（分红子项因缺少对应同源 EOD 快照跳过），详见
   `deploy/agentctl-q0/real-flow-result.json`。
...
8. **S2 正式财务事实链**——BaoStock 缺 F07/F09 所需绝对值，Tushare 因付费权限退出。巨潮对 8 家普通工商企业 64/64 期已完成公告关联；601012、600519、000333 的五期官方 PDF 各有 16/16 个 S2 必需候选值，累计 **48/48 候选、0/48 正式 PIT 事实**。三只样本的 15 个证券—期间组合已完成定期报告和全分类公告索引。601012 的一对年报修订关系有更正公告证明；600519 和 000333 两只正分母样本的 F07–F09 可由原文诊断重算，并可在本机工作台独立的**未审阅公式预览**中查看；F10、排名、回测和 S2 注册仍关闭。定期报告与全分类索引的官方组织 ID、请求体、分页和公告关联现可归档复核；逐值、逐更正候选审阅的持久准入层已实现。601012 的全分类真实样本已检出定期报告分类漏掉的更正公告；实际人工复核、其它期间更正链、合格证券池覆盖、决策日 F10 和金融行业独立模板尚未通过。CNINFO 本地研究及保存已按公开披露登记，不能扩大解释为商用或再分发许可。上交所结构化概览缺合并净利润与归母权益，且 600519“营业总收入”不能冒充 F09 营业收入，见 ADR-015。S2 继续关闭；领域层和 API 阻止新建 `family=S2`，该项不阻断 S1 首期试运行。
