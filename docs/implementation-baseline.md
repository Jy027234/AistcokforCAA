...
8. **F07/F09 绝对财务值**——BaoStock 当前缺失；Tushare 因付费权限退出当前方案。免费路线已实现新浪三表受限解析器（2 只样本、各 5 个共同报告期字段完整）和 mootdx 财务包探针（5,568 行、585 列），巨潮与沪深交易所原始披露负责公告时点与修订证据。当前仍缺全市场/行业覆盖、显式公告关联、真实修订链和权利审阅，且 mootdx 单季字段有 22–25 个零值歧义，因此 S2 继续关闭，见 ADR-005、ADR-015。领域层和 API 会阻止新建 `family=S2`，并通过 `familyGates` 返回关闭原因。该项不阻断 S1 首期试运行。
