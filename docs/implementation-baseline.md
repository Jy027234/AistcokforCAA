...
10. **上市天数门槛只能判一半**（§8.7）——窗口在 120 个交易日以内的快照
...
| 上市日在快照窗口**之内** | 窗口内交易日数 < 门槛则排除（`LISTED_TOO_RECENTLY`） | 单调：窗口内不够，则一定不够 |
| 上市日在窗口起点**之前** | **不排除**，写一条 note | 61 天的窗口判断不了 120 个交易日 |

第三条容易被误当成"实现不完整"，但另两种做法都是本项目已经踩过的错：
当成"不合格"会清空整池，当成"合格"而不留痕是拿未知做准入判断。
