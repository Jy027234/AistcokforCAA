...
| T12 双重验收 | 🟡 | 量化侧报告已成；最新 Q5 为 11 通过、1 失败、4 未覆盖，9 项 live 通过；A02 doctor 失败，A01/A06/A09/A14 未覆盖 |
...
2. **Q5 接入侧验收仍未关闭**——`docs/integration/q5-acceptance-report.md`
   ...
   总计 11 通过、1 失败、4 未覆盖，真实拓扑通过
   9 项；A01 观察到 `mode_compatible=false` 后调用仍被接受；A06 因模型请求未完成而未覆盖；
   A09 缺确认/冻结入口；A14 已接好严格解析、发布和独立分页读取，但当前 Lite server 没有
   Platform Core `conversation_replay` 传输，所以保持 fail closed 与未覆盖。A02 因当前 manifest
