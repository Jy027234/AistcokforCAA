...
| **券商佣金** | **无统一权威值** | **合同约定** | 优先按使用者实际合同；试运行假设必须由使用者明确批准并单独标记 | 可回读来源标记 |
...
**佣金为什么不做静默默认值：** 替使用者填一个数字，等于又制造一处
「看起来有依据的事实」。因此它是显式参数（`AQUANT_COMMISSION_RATE` 与
`AQUANT_COMMISSION_MIN_CENTS`）。未配置时标为 `UNCONFIGURED_DEFAULT` 并拒绝真实模拟；
若使用者明确批准行业常用值先做试运行，则必须同时标
`USER_APPROVED_ASSUMPTION`，允许试运行但不能把结果称为实际券商账单。

2026-09-19，使用者明确批准以万分之 2.5、最低 5 元作为当前本机试运行假设。
选择依据是证监发〔2002〕21 号确认 A 股佣金在上限内协商、每笔不足 5 元按 5 元收取，
以及证监局公开案例记录过万分之 2.5 的开户佣金。该授权只确定当前试运行参数，不证明
使用者实际账户合同费率；本机配置放在不入库的 `real-trial.env`，后续可替换。
