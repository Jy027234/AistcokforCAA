...
| `baostock` | 公开披露 | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| `cninfo` | 公开披露 | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| `sse-site` / `szse-site` | 公开披露 | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| `tencent-*` / `sina-hq` / `eastmoney-direct` | **使用者授权** | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |

### 2026-09-15：消费级网站由使用者确认放行

使用者在了解"这一组最需要本人核对条款"之后，确认放行**研究使用与本地保存**。
登记表以 `basis=USER_AUTHORIZED` 记录。

**这个 basis 的含义必须说准**：它记录的是"谁在什么时候做了这个决定"，
**不是**"条款已经法律审阅过"。把使用者的一句话记成 `TERMS_REVIEWED`，
等于替使用者做了一次并没有做过的法律审阅，而后来的人无法分辨两者。

**授权范围只到研究使用与本地保存。** 两项没有跟着放开：

* `excerpt_display`（片段展示）——展示片段涉及转载，是另一类风险；
* `model_processing`（模型外发）——使用者确认的是"条款可接受"，
  不是"数据可以发给第三方"。这两件事的风险量级不同，
  不应由我替他扩展。实测确认：授权后各来源的 `model_processing`
  仍为 `UNKNOWN`，外发闸门对全部真实来源仍返回拒绝。
...
3. **`model_processing` 一律 ❌（全部来源）**
   ...
   2026-09-15 的使用者授权**没有**改变这一项。若要放开，需要的是
   针对"发送至第三方处理"的依据，而不是"条款可接受"的判断。
   两者不是一个问题：前者管的是数据离开本机之后的去向。
